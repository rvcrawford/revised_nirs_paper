...
This is a more lax standard for RPD than that set forth by @williams_calibration_2013, who recommended a minimum acceptable RPD value of 2.4 as suitable for use rough screening, such as breeding programs. Utilizing those higher benchmarks, `r round(final_model_analysis$metrics[rpd>2.4] |> nrow()/10)`% of the models were sufficient for rough screening and `r round(final_model_analysis$metrics[rpd>3.1] |> nrow()/10)`% were suitable for screening, although none sufficed for quality or process control. However, it models with lower RPD values can still be of use to researchers [@bellon-maurel2010]

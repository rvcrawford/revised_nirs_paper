...
library(caret)
  ...
  head(dat)
my_comparison <- function(data, full_data){
    case_weights = data$sample_weights
    spectra_matrix = data |> tibble() |> select(starts_with("x"))  
    ...
    # Specify the desired number of PLS components
    n_components <- 12  # Replace with the number of components you want
    
    # Create a tuneGrid with the specific number of components
    pls_grid <- expand.grid(.ncomp = n_components)
    
    # Fit the PLS model using train with the specified tuneGrid
    pls_model <- train(
      x = final_spectra_matrix[inds,],
      y = y[inds],
      method = "pls",  # Specify the PLS method
      tuneGrid = pls_grid,  # Provide the tuneGrid with the specified number of components
      trControl = trainControl(method = "cv", number = 5)  # Example: 5-fold cross-validation
    )
    
    pls_model_case_weights <- train(
      x = final_spectra_matrix[inds,],
      y = y[inds],
      weights = case_weights[inds],
      method = "pls",  # Specify the PLS method
      tuneGrid = pls_grid,  # Provide the tuneGrid with the specified number of components
      trControl = trainControl(method = "cv", number = 5)  # Example: 5-fold cross-validation
    )
    ...
    print(pls_model)
    print(pls_model_case_weights)
    
    nocase = predict(pls_model,final_spectra_matrix[-inds,])
    has_case = predict(pls_model_case_weights,final_spectra_matrix[-inds,])
    
    tibble(unweighted = nocase, weighted = has_case)
    # predict(pls_model_case_weights,final_spectra_matrix[-inds,]))
...
my_comparison_weighted_sampling <- function(data, full_data, seed = 123) {
  ...
  detailed_predictions <- data.frame(
    iteration = seed,  # Use seed as iteration identifier
  ...
  return(list(
    ...
    improvements = weighted_metrics - unweighted_metrics,  # Note: rsq improvement is weighted - unweighted
